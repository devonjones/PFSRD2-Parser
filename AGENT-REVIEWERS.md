...
- **`silent-failure-hunter`** — covered jointly by `error-handling-reviewer` and `strategic-fragility-reviewer`. The error-handling reviewer flags silent exception swallows (P1), generic `except Exception` without re-raise (P2), bare `except:` (P2), and missing `raise ... from` (P3). The parser-specific strategic-fragility-reviewer goes further — every defensive default that could mask malformed HTML is flagged. Combined, our pack catches everything silent-failure-hunter would, more aggressively.
...
A reviewer pack for the pfsrd2-parser project (Python parsers: HTML → structured JSON, BeautifulSoup-heavy, fail-fast strategic-fragility philosophy). Each section below is an independent reviewer prompt with its own scope, set of patterns to flag, and disposition.
...
4. **Test file naming and structure.** Tests live in `tests/test_<module>.py` matching the module being tested (e.g. `pfsrd2/armor.py` → `tests/test_armor.py`). Recognize as valid coverage:
...
Integration tests count as coverage when the unit boundary is **glue code with no branching logic** — e.g., a Click command that parses args and calls one function, or a pipeline pass that decodes input and dispatches to a helper. Demanding a separate unit test for such glue produces low-value mock-heavy tests.

For parser code specifically, integration tests that exercise the full pipeline (HTML → JSON, single-file or fixture-driven) are common and appropriate — flag a missing unit test only when the unit being touched has its own branching, validation, parsing, or business logic that can be genuinely tested in isolation. The fail-fast strategic-fragility philosophy means many parser helpers are best-exercised by feeding them real HTML through the pipeline; a mock-heavy unit test that bypasses BeautifulSoup adds little signal.
...
1. **`mock.patch` at point-of-definition instead of point-of-use:**
   ...
   # BAD — doesn't intercept the call inside pfsrd2.module
   @mock.patch("os.path.exists")
   def test_check(mock_exists):
       ...

   # GOOD — patches the symbol that pfsrd2.module actually consults
   @mock.patch("pfsrd2.module.os.path.exists")
   def test_check(mock_exists):
       ...
   ```

   `pfsrd2.module` did `import os` at module load — it has its OWN reference to `os.path.exists`. Patching `os.path.exists` rebinds the original module's attribute, but pfsrd2.module's local-bound reference still points at the original function. This is the single most common reason `mock.patch` "doesn't work."
...
2. **Mocking what you don't own:**
   ...
   @mock.patch("pfsrd2.http_client.fetch")
...
6. **Custom exception classes for caller-handleable cases (P3):**
   ...
   class ParserError(Exception):
       """Base class for parser-layer errors."""

   class EntryNotFound(ParserError):
       pass
...
**Cross-reference:** For this parser, see also `strategic-fragility-reviewer` which extends these rules with parser-specific aggressive fail-fast patterns (defensive defaults that mask missing HTML structure, fallback logic that hides structural problems, etc.).
...
1. **"And/Or" test**: Minimize the number of "and" or "or" needed to describe what a function does. If you need multiple conjunctions, the function is doing too much.
   ...
   - Bad: "This function parses the config AND normalizes values AND adds defaults AND handles edge cases for variants."
...
Review code to ensure it follows the **strategic fragility** design philosophy. This parser deliberately uses fail-fast, assertion-heavy patterns because **wrong data is worse than no data**. The parser should fail loudly and immediately when encountering unexpected conditions, rather than silently producing incorrect data.

This reviewer is parser-specific and **extends** the universal `error-handling-reviewer`. It treats defensive patterns that would be benign in a web app (`get("name", "Anonymous")`) as actively harmful here, because they mask structural HTML problems that need human investigation.

**Reference:** See `docs/architecture.md` section "Design Philosophy: Strict Validation Over Graceful Degradation" for full context.

**Patterns to FLAG:**

1. **Silent exception swallowing (P1):**
...
3. **Overly defensive defaults that mask missing data (P2):**
...
4. **Generic exception handling without re-raising (P2):**
...
5. **Fallback logic that hides structural problems (P2):**
...
6. **Defensive `.get()` / `or {}` / `if x else default` in code paths that consume parser output (P2):**

   When extracting from BeautifulSoup or walking a parsed structure, prefer direct indexing or attribute access that raises on the unexpected. The HTML and intermediate dicts have known shapes; defensive lookups silently produce different shapes downstream.

   ```python
   # BAD
   trait_name = link.get('title') or link.get_text() or "Unknown"

   # GOOD — if title is missing, that's a parser bug or HTML bug; surface it
   trait_name = link.get('title')
   assert trait_name, f"Trait link missing title: {link}"
   ```
...
5. **Well-documented intentional exceptions:**
...
- If the parser silently handles bad HTML, bugs propagate to output data.
...
- HTML bugs should be fixed in pfsrd2-web, not worked around in parser code.

**Review approach:**

1. Search for `try/except` blocks — evaluate if they're swallowing errors.
2. Check for `return` statements inside `finally` blocks.
3. Look for defensive defaults like `x if y else {}` or `dict.get(key, fallback)` that could mask missing data.
4. Identify bare `except:` without `raise`.
5. Flag generic `except Exception` that doesn't re-raise.
6. Ask of every defensive pattern: "If the HTML is malformed here, would this code silently produce wrong data?" If yes and there's no documentation explaining the intentional choice, flag it.
...
Review code for duplication of shared utilities. The pfsrd2-parser project has a `universal/` package containing common helpers used across all parsers; reimplementing them inline creates drift and bypasses battle-tested edge-case handling.

Before reviewing, READ these files to understand what's available:
...
1. **Reimplementing a `universal/` function inline** — flag with the specific function that should be used instead. Suggest the one-line replacement.

2. **Near-duplicates that diverge in subtle ways** — e.g., a custom paren-aware split that handles `[` `]` instead of `(` `)`. Often the right fix is to extend the universal function with a parameter, not to keep a parallel implementation.

3. **Parser-local utility files that should live in `universal/`** — if a helper in `pfsrd2/<type>.py` is generic (no type-specific logic, uses only BeautifulSoup/stdlib) and could plausibly help another parser, recommend moving it.

**Do NOT flag:**

- Type-specific extraction logic that happens to look superficially similar to a universal helper.
- One-off uses where the universal helper would require awkward parameter passing.
...
Review JSON schema changes in `pfsrd2/schema/` for consistency with existing patterns. The schemas are documentation contracts that consumers rely on; drift between sibling schemas causes integration bugs. Before reviewing, READ the existing schemas to understand established conventions.

**Patterns to FLAG:**

1. **Arrays of bare primitives where typed objects are the established pattern (P2):**

   Arrays should be arrays of objects with `type` and `subtype` properties, not arrays of primitives — unless an existing pattern in sibling schemas already uses primitives for that field.
   ...
   // GOOD
   "traits": [
     {"type": "trait", "name": "fire", "link": "/Traits.aspx?ID=1"},
     {"type": "trait", "name": "magical", "link": "/Traits.aspx?ID=2"}
   ]
   ```

2. **Superset fields on shared concepts (P2):**

   If an object represents the same concept as one in another schema (e.g., `link`, `source`, `ability`), it is **fine to have a subset** of the fields defined elsewhere — not every schema needs every optional field. However, flag any **superset fields** (fields that don't exist in any other schema's version of the same concept), as these may indicate typos, naming divergence, or unintentional additions. Enum values can differ, but field names and types for shared fields should match.

3. **Inline duplication instead of `$ref` (P3):**

   Common objects should be defined in `#/definitions` and referenced with `$ref`, not duplicated inline. If the same object shape appears in three or more places, factor it into a `#/definitions` entry.

4. **Missing or incorrect `required` fields (P2):**

   Objects should declare appropriate `required` fields matching the pattern of similar objects elsewhere. A schema that says no fields are required but whose parser populates them always is misleading consumers.

**Acceptable patterns (do NOT flag):**

1. **Cardinality differences are legitimate:**

   The same field name (e.g., `saving_throw`) may have different cardinality (single object vs array) in different contexts. For example, an `ability` may have multiple saving throws (primary + escape DC), while an `affliction` always has exactly one. This is intentional — different game concepts have different mechanical needs. Do NOT flag cardinality differences between sibling definitions unless the rationale is unclear.

2. **Schema-version-pinned shapes:** A field that exists in `schema1.2` but is intentionally absent from `main` (or vice versa) reflects deliberate evolution, not drift.
...
4. Verify arrays use object wrappers with type/subtype, not bare primitives — unless the surrounding schemas already established the primitive pattern for that field.
...
data_file = resources.files("pfsrd2.data").joinpath("config.json")
config = json.loads(data_file.read_text())
```

For Python ≥3.9. On older versions, `importlib.resources.read_text("pfsrd2.data", "config.json")` is the equivalent.
...
| `strategic-fragility-reviewer` | `*.py` production code in `pfsrd2/` |
...
| `shared-utils-reviewer` | `*.py` in `pfsrd2/` (skips `tests/` and `universal/` itself) |
| `schema-consistency-reviewer` | `pfsrd2/schema/*.json` |
| `import-reviewer` | `*.py` |
...
- `strategic-fragility-reviewer`: **P2** by default; **P1** for silent exception swallows and `return` in `finally` (same severity profile as error-handling-reviewer).
...
- `test-coverage-reviewer`, `external-process-reviewer`, `importlib-resources-reviewer`, `typing-consistency-reviewer`, `schema-consistency-reviewer`: **P2** by default; specific flags inside each reviewer may be P1.
- `complexity-reviewer`: **P2** for mccabe floor violations; **P3** for heuristic findings.
- `import-reviewer`, `dead-code-reviewer`, `clarity-reviewer`, `shared-utils-reviewer`: **P3** by default.
