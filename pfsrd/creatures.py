...
def handle_special_attacks(_, elem, text):
    ...
    for text in parts:
        ...
        if text.find("(") > -1:
            ...
            modparts = split_comma_and_semicolon(
                modtext, parenleft="[", parenright="]")
...
def defense_pass(struct):
    ...
    def _handle_hp(_, defense, text):
        ...
        hp['healing_abilities'] = modifiers_from_string_list(
            parts, "healing_ability")
        ...
        def _handle_spell_list(text):
            def _handle_spell_list_deets(deets):
                ...
                if len(deetparts) == 1:
                    ...
                    if len(spell_level_text) == 1:
                        ...
                        assert False, deets
                    spell_list['count_text'] = deetparts[1].replace(
                        ")", "").strip()
                else:
                    assert False, deets
            ...
            def _handle_spell(spell_list, text):
                ...
                if (first.endswith(":")):
                    ...
                    bslist.pop(0)
                spell['name'] = prefix + \
                    clear_tags(str(bslist.pop(0)), ['i']).strip()
                ...
                if len(bslist) > 0:
                    ...
                    if spell['name'] == "summon":
                        exttext = get_text(
                            BeautifulSoup(exttext, 'html.parser'))
                    ...
                    if len(exttext.strip()) > 0:
                        ...
                        if m:
                            ...
                            spell['name'] = "%s %s" % (
                                spell['name'], spell_version)
                        assert exttext.startswith(
                            "(") and exttext.endswith(")"), exttext
                        exttext = exttext[1:-1]
                        ...
                        for extpart in extparts:
                            if extpart.endswith("level"):
                                ...
                                parts = extpart.split(" ")
                                parts.pop(0)
                                spell['range'] = universal_handle_range(
                                    " ".join(parts))
                            elif extpart.startswith("DC"):
                                spell['saving_throw'] = universal_handle_save_dc(
                                    extpart)
                        ...
                        if len(modlist) > 0:
                            spell['modifiers'] = modifiers_from_string_list(
                                modlist)
                return spell
            ...
            for spelltext in spelltexts:
                spell_list['spells'].append(
                    _handle_spell(spell_list, spelltext))
                    ...
                    if damagepart.find('[') > -1:
                        ...
                        for modpart in modparts:
                            if modpart.find("DC") > -1:
                                attack_damage['saving_throw'] = universal_handle_save_dc(
                                    modpart)
                            elif modpart.endswith("ft."):
                                attack_damage['range'] = universal_handle_range(
                                    modpart)
                            else:
...
def statistics_pass(struct):
    ...
    def _handle_bab(name):
        def _handle_bab_impl(struct, _, text):
            ...
            offense[name.lower()] = number_with_modifiers(
                text.strip(), name.lower())
    ...
    def _handle_sq(_, statistics, text):
        ...
        sqs = string_with_modifiers_from_string_list(
            ...
            "special_quality")
        for sq in sqs:
            assert sq['name'].lower(
            ) != 'tricks', "Tricks should be broken out %s" % sq
            ...
            for key in ecology.keys():
                if key in struct["stat_block"]['ecology']:
                    assert ecology[key] == struct['stat_block']['ecology'][key], "%s : %s" % (
                        ecology, struct['stat_block']['ecology'])
                else:
...
def special_ability_pass(struct):
    def _handle_special_abilities(sa_section):
        ...
        special_abilities = struct['stat_block'].setdefault(
            'special_abilities', [])
        other_abilities = struct['stat_block'].setdefault(
            'other_abilities', [])
    ...
    def _handle_affliction(sa):
        ...
        for part in parts:
            ...
            if title == "save":
                ...
                affliction['affliction_type'] = "%s; %s" % (
                    sa["name"].strip(), text.strip())
            else:
