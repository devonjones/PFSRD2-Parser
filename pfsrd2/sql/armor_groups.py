...
def create_armor_groups_index(curs):
    sql = "\n".join(["CREATE INDEX armor_groups_game_id", " ON armor_groups (game_id)"])
    curs.execute(sql)


def truncate_armor_groups(curs):
    sql = "\n".join(["DELETE FROM armor_groups"])
    curs.execute(sql)
...
def fetch_armor_group_by_name(curs, name):
    values = [name.lower()]
    sql = "\n".join(["SELECT ag.*", " FROM armor_groups ag", " WHERE ag.name = ?"])
    curs.execute(sql, values)


def fetch_armor_group_by_game_id(curs, game_id):
    values = [game_id]
    sql = "\n".join(["SELECT *", " FROM armor_groups", " WHERE game_id = ?"])
    curs.execute(sql, values)
