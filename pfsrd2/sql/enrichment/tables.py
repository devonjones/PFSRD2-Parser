...
def create_ability_records_index(curs):
    curs.execute(
        "CREATE INDEX ability_records_identity_hash"
        " ON ability_records (identity_hash)"
    )
    curs.execute(
        "CREATE INDEX ability_records_name"
        " ON ability_records (name)"
    )
    curs.execute(
        "CREATE INDEX ability_records_stale"
        " ON ability_records (stale)"
    )
    curs.execute(
        "CREATE INDEX ability_records_enrichment_version"
        " ON ability_records (enrichment_version)"
    )
...
def create_ability_creature_links_index(curs):
    curs.execute(
        "CREATE INDEX ability_creature_links_ability_id"
        " ON ability_creature_links (ability_id)"
    )
    curs.execute(
