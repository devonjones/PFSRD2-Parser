...
def create_monster_abilities_index(curs):
    sql = "\n".join(["CREATE INDEX monster_abilities_game_id", " ON monster_abilities (game_id)"])
    curs.execute(sql)


def truncate_monster_abilities(curs):
    sql = "\n".join(["DELETE FROM monster_abilities"])
    curs.execute(sql)
...
def insert_monster_ability(curs, monster_ability):
    ...
    values = [
        monster_ability["game-id"],
        monster_ability["name"].lower(),
        json.dumps(monster_ability.get("classes", [])),
        text,
    ]
    sql = "\n".join(
        [
