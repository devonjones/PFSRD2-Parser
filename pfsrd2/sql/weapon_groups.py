...
def create_weapon_groups_index(curs):
    sql = "\n".join(["CREATE INDEX weapon_groups_game_id", " ON weapon_groups (game_id)"])
    curs.execute(sql)


def truncate_weapon_groups(curs):
    sql = "\n".join(["DELETE FROM weapon_groups"])
    curs.execute(sql)
...
def fetch_weapon_group_by_name(curs, name):
    values = [name.lower()]
    sql = "\n".join(["SELECT wg.*", " FROM weapon_groups wg", " WHERE wg.name = ?"])
    curs.execute(sql, values)


def fetch_weapon_group_by_game_id(curs, game_id):
    values = [game_id]
    sql = "\n".join(["SELECT *", " FROM weapon_groups", " WHERE game_id = ?"])
    curs.execute(sql, values)
