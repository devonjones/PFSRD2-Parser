...
def restructure_alien_pass(details, subtype):
    ...
    def _handle_sources(top):
        ...
        for link in links:
            ...
            link.extract()
        assert str(bs).replace("<b>Source</b>",
                               "").replace(",", "").strip() == "", str(bs)
        del top['text']
        return retarr
    ...
    def _handle_defensive_abilities(defense, daparts):
        ...
        for dapart in daparts:
            ...
            if name == "SR":
                ...
                defense['defensive_abilities'] = universal_handle_defensive_abilities(
                    value)
            else:
                ...
                assert False, "Unexpected section in Defensive Abilities: %s: %s" % (
                    name, value)
        ...
        def _handle_attack_impl(offense, _, text):
                ...
                if damagetext.find('[') > -1:
                    ...
                    for modpart in modparts:
                        if modpart.find("DC") > -1:
                            attack['saving_throw'] = universal_handle_save_dc(
                                modpart)
            ...
            for attacktext in attacks:
                ...
                if len(attackparts) == 1:
                    _handle_attack_damage(
                        attackparts.pop().replace(")", "").strip())
                melee.append(attack)
